...
class CartsFile {
    ...
    getById = (number, user) => {
        const arrayList = JSON.parse(fs.readFileSync('../Segunda_entrega/dataBases/data/listCarts.txt', 'utf-8'));
        let exists=0;
        let locate=0;
        for (let index = 0; index < arrayList.length; index++) {
            if (parseInt(number) === parseInt(arrayList[index].idCart)) {
    ...
    includeProductById = async (idCart, id, user) => {
        ...
        if (existsCart===1) {
            ...
            if (arrayList[locateCart].user === user || user === "admin") {
                ...
                const arrayProducts = JSON.parse(fs.readFileSync('../Segunda_entrega/dataBases/data/listProducts.txt', 'utf-8'));
